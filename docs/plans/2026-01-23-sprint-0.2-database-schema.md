...
4. DATABASE_URL in `.env.local`: `postgresql://USER:PASSWORD@localhost:5432/velikibukovec`
...
Or with Docker:
```bash
docker run --name vb-postgres -e POSTGRES_PASSWORD=CHANGE_ME -e POSTGRES_DB=velikibukovec -p 5432:5432 -d pgvector/pgvector:pg15
```
